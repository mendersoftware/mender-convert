...
_copy_boot_mender() {
	...
	generate_line 80 '='
	echo_broadcast "Copying boot: ${source}p1 -> ${boot_partition}"
        rsync -aAxv $rsync_options /boot/efi/* ${tmp_boot_dir} || write_failure
        cp grub_emmc.cfg ${tmp_boot_dir}/EFI/BOOT/grub.cfg
}
...
_copy_rootfs_a_mender() {
  empty_line
  generate_line 80 '='
  echo_broadcast "Copying: rootfs_a to ${rootfs_partition}"
  generate_line 40
  echo_broadcast "==> rsync: / -> ${tmp_rootfs_dir}"
  generate_line 40
  get_rsync_options
  ...
  cp /opt/scripts/tools/eMMC/fstab_mender ${tmp_rootfs_dir}/etc/fstab
...
_copy_rootfs_b_mender() {
  empty_line
  generate_line 80 '='
  echo_broadcast "Copying: rootfs_b to ${destination}p3"
  generate_line 40
  echo_broadcast "==> rsync: / -> ${tmp_rootfs_dir}"
  generate_line 40
  get_rsync_options
  ...
  echo_broadcast "Copying: Current rootfs to ${destination}p3 complete"
  generate_line 80 '='
  empty_line
  generate_line 80 '='
  echo_broadcast "Final System Tweaks:"
...
_copy_datafs_mender() {
  ...
  echo_broadcast "mounting ${destination}p4 to /data"
  mount /dev/mmcblk0p4 /data || true

  echo_broadcast "Copying: datafs to ${destination}p4"
  generate_line 40
  echo_broadcast "==> rsync: /data -> ${tmp_rootfs_dir}"
  generate_line 40
  get_rsync_options
  rsync -aAx $rsync_options /data/* ${tmp_rootfs_dir} --exclude={/lost+found} || write_failure
  flush_cache
  generate_line 40
  echo_broadcast "Copying: datafs to ${destination}p4 complete"
  generate_line 80 '='
  empty_line
}
...
partition_device_mender() {
  ...
  if [ "x${boot_fstype}" = "xfat" ] ; then
    ...
    sfdisk_boot_startmb="${conf_boot_startmb}M"
    ...
    echo_broadcast "source: ${source}"
    echo_broadcast "destination: ${destination}"

    # Derive each sfdisk param here.
    # After my experiments, it turned out that sfidks intervenes into the 
    # desired partition layout because of boot start offset. 
    # This way, after reproducing the problem and some googling,
    # I following the advice specify the partition layout directly in blocks 
    # and not giving any chance to sfdisk to change it.
    # 
    # sfdisk_boot_start_blocks, sfdisk_boot_size_blocks
    # sfdisk_rootfs_a_start_blocks, sfdisk_rootfs_a_size_blocks,
    # sfdisk_rootfs_b_start_blocks, sfdisk_rootfs_b_size_blocks,
    # sfidk_data_start_blocks, sfdisk_data_size_blocks

    #boot
#    sfdisk_boot_start_blocks=$((${conf_boot_startmb} * 1024 * 1024 / 512))
#    sfdisk_boot_size_blocks=$((${conf_boot_endmb} * 1024 * 1024 / 512))
    sfdisk_boot_start_blocks=$(cat /sys/class/block/$(basename ${source})p1/start)
    sfdisk_boot_size_blocks=$(cat /sys/class/block/$(basename ${source})p1/size)
    
    #rootfs_a
    sfdisk_rootfs_a_start_blocks=$((${sfdisk_boot_start_blocks} + ${sfdisk_boot_size_blocks}))
    sfdisk_rootfs_a_size_blocks=$(cat /sys/class/block/$(basename ${source})p2/size)

    #rootfs_b
    sfdisk_rootfs_b_start_blocks=$((${sfdisk_rootfs_a_start_blocks} + ${sfdisk_rootfs_a_size_blocks}))
    sfdisk_rootfs_b_size_blocks=$(cat /sys/class/block/$(basename ${source})p3/size)

    #data
    sfdisk_data_start_blocks=$((${sfdisk_rootfs_b_start_blocks} + ${sfdisk_rootfs_b_size_blocks}))
    ...
    echo_broadcast "==> sfdisk parameters:"
    echo_broadcast "sfdisk_boot_start_blocks = ${sfdisk_boot_start_blocks}"
    echo_broadcast "sfdisk_boot_size_blocks = ${sfdisk_boot_size_blocks}"
    echo_broadcast "sfdisk_rootfs_a_start_blocks = ${sfdisk_rootfs_a_start_blocks}"
    echo_broadcast "sfdisk_rootfs_a_size_blocks = ${sfdisk_rootfs_a_size_blocks}"
    echo_broadcast "sfdisk_rootfs_b_start_blocks = ${sfdisk_rootfs_b_start_blocks}"
    echo_broadcast "sfdisk_rootfs_b_size_blocks = ${sfdisk_rootfs_b_size_blocks}"
    echo_broadcast "sfdisk_data_start_blocks = ${sfdisk_data_start_blocks}"

    echo_broadcast "sfdisk: [$(LC_ALL=C sfdisk --version)]"
    echo_broadcast "sfdisk: [sfdisk ${sfdisk_options} ${destination}]"
    echo_broadcast "sfdisk: [${sfdisk_boot_startmb},${sfdisk_boot_size_mb},${sfdisk_fstype},*]"
    echo_broadcast "sfdisk: [,${sfdisk_rootfs_a_size_mb},${sfdisk_fstype},]"
    echo_broadcast "sfdisk: [,${sfdisk_rootfs_b_size_mb},${sfdisk_fstype},]"
    echo_broadcast "sfdisk: [,,${sfdisk_fstype},]"
    echo_broadcast "==> Partitionning"
    
    generate_line 60
    LC_ALL=C sfdisk ${sfdisk_options} "${destination}" <<__EOF__
${sfdisk_boot_start_blocks},${sfdisk_boot_size_blocks},c,*
${sfdisk_rootfs_a_start_blocks},${sfdisk_rootfs_a_size_blocks},L,
${sfdisk_rootfs_b_start_blocks},${sfdisk_rootfs_b_size_blocks},L,
${sfdisk_data_start_blocks},,L,
__EOF__
...
_prepare_future_rootfs_a_mender() {
	empty_line
	generate_line 80 '='
	echo_broadcast "Preparing future rootfs_A to receive files"
	generate_line 40

        empty_line
        echo_broadcast "==> Formatting rootfs_A with mkfs.ext4 ${rootfs_partition} -F -L ROOTFS_A"
        generate_line 80
        empty_line
        LC_ALL=C mkfs.ext4 ${rootfs_partition} -F -L "ROOTFS_A"
        generate_line 80
        echo_broadcast "==> Formatting rootfs: ${rootfs_partition} complete"
        flush_cache
...
_prepare_future_rootfs_b_mender() {
	empty_line
	generate_line 80 '='
	echo_broadcast "Formatting future rootfs_B"
	generate_line 40

        empty_line
        echo_broadcast "==> Formatting rootfs_B with mkfs.ext4 ${destination}p3 -F -L ROOTFS_B"

        generate_line 80
        empty_line
        LC_ALL=C mkfs.ext4 ${destination}p3 -F -L "ROOTFS_B"
        generate_line 80
        echo_broadcast "==> Formatting rootfs B  complete"
        flush_cache

	tmp_rootfs_dir=${tmp_rootfs_dir:-"/tmp/rootfs"}
	echo_broadcast "==> Creating temporary rootfs directory (${tmp_rootfs_dir})"
	mkdir -p ${tmp_rootfs_dir} || true
	echo_broadcast "==> Mounting ${destination}p3 to ${tmp_rootfs_dir}"
	mount ${destination}p3 ${tmp_rootfs_dir} -o async,noatime
	empty_line
	generate_line 80 '='
}

_prepare_future_datafs_mender() {
	empty_line
	generate_line 80 '='
	echo_broadcast "Formatting future rootfs datafs"
	generate_line 40

        empty_line
        echo_broadcast "==> Formatting datafs with mkfs.ext4 ${destination}p4 -F -L DATA"

        generate_line 80
        empty_line
        LC_ALL=C mkfs.ext4 ${destination}p4 -F -L "DATA"
        generate_line 80
        echo_broadcast "==> Formatting datafs  complete"
        flush_cache

	tmp_rootfs_dir=${tmp_rootfs_dir:-"/tmp/rootfs"}
	echo_broadcast "==> Creating temporary data directory (${tmp_rootfs_dir})"
	mkdir -p ${tmp_rootfs_dir} || true
	echo_broadcast "==> Mounting ${destination}p4 to ${tmp_rootfs_dir}"
	mount ${destination}p4 ${tmp_rootfs_dir} -o async,noatime
	empty_line
	generate_line 80 '='
}
...
# This function differs from original prepare_mender() by the following:
# - creates rootfs B partition (add it to a parition table, format and provide fstab)
# - creates data partition (add it to a partition table and copy the contents from SD card /data)
# - partition table is hardcoded to fit into eMMC (3600MB) capacity
prepare_drive_mender() {
  empty_line
  ...
  _prepare_future_rootfs_a_mender
  _copy_rootfs_a_mender
  # works for any non-boot partition
  _teardown_future_rootfs
